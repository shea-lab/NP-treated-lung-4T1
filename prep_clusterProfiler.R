...
#' @param min_pct min percent of cells that express for FindMarkers, default 0.1 
#' @param logfc_thresh  logfc threshold for FindMarkers , default 0.25
#' 
#' @return results_list list of clusterProfiler GO, KEGG, DO, and Pathway clusters



prep_clusterProfiler <- function(sample, organism,  min_pct = 0.1,  logfc_thresh = 0.25) {
  # load libraries 
  library(Seurat)
  ...
  markers <- FindAllMarkers(sample, only.pos = TRUE, min.pct = min_pct, logfc.threshold = logfc_thresh)
  ...
  # return(genelist)
  results_list <- list(genelist, markers)
  return(results_list)
