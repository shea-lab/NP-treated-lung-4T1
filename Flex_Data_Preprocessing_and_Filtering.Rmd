...
```{r source files}
source("~/Documents/Kate/NMMscRNAseq/remove_soup.R")
source("~/Documents/Kate/NMMscRNAseq/find_doublets.R")
source("~/Documents/Kate/NMMscRNAseq/run_ddqcR.R")
```
...
```{r}
# data_dir = "/Users/kategriffin/Documents/Shea Lab/NP Mechanisms in SCI/Flex_Seq/10340-SH/10x_summary_10340-SH/"
...
** loop over all samples ** 4/18 KG
cannot run twice because you end up writing over the original file and it says specified 'path' already exists
need to add if statement to remove if it exists then re-save 
```{r run soupX}
...
saveRDS(data, file = "~/Documents/Kate/NMMscRNAseq/Data/merged_sct_filtered_tnbc_object.rds")
```
