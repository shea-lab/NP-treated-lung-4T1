...
v2 script uses updated cellchat objects without dropped unknowns (unknowns were ID'd) 4/7/25
...
labels.levels <- c("Neut", "Infl Neut","IFN Neut","Baso","NK","B", "Mac", "recMac", "alvMac", "moDC","Th1", "Tcm", "HSP T", "AT2",    "Endo",  "Fib",  "4T1")
...
# use all CellChatDB for cell-cell communication analysis
CellChatDB.use <- CellChatDB # simply use the default CellChatDB. We do not suggest to use it in this way because CellChatDB v2 includes "Non-protein cellchat_Treat" (i.e., metabolic and synaptic signaling). 
...
```{r}
# Compute Communicaiton probability and infer network ---------------------
cellchat_Treat <- computeCommunProb(cellchat_Treat, population.size = T) #remove pop.size = F
#> triMean is used for calculating the average gene expression per cell group. 
#> [1] ">>> Run CellChat on sc/snRNA-seq data <<< [2022-11-26 08:34:38]"
#> [1] ">>> CellChat inference is done. Parameter values are stored in `object@options$parameter` <<< [2022-11-26 08:35:36]"
# Filter out the cell-cell communication if there are only few number of cells in certain cell groups
...
cellchat_Untreat <- readRDS("~/NMMscRNAseq/Data/Cellchat/cellchat_Untreat.rds")
cellchat_Treat <- readRDS("~/NMMscRNAseq/Data/Cellchat/cellchat_Treat.rds")
```
